...
import { X, ShieldAlert, ArrowRight, LayoutGrid, List } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div>
          <div className="flex items-center justify-between">
            ...
            {/* Mobile View Toggle */}
            <div className="flex sm:hidden items-center bg-slate-100 p-0.5 rounded-lg text-[11px] font-medium">
              <button
                type="button"
                onClick={() => setMobileView('grid')}
                className={`px-2 py-1 rounded-md flex items-center gap-1 ${
                  mobileView === 'grid' ? 'bg-white shadow-xs text-blue-700 font-semibold' : 'text-slate-600'
                }`}
              >
              ...
              <button
                ...
                className={`px-2 py-1 rounded-md flex items-center gap-1 ${
                  mobileView === 'list' ? 'bg-white shadow-xs text-blue-700 font-semibold' : 'text-slate-600'
                }`}
              >
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Likelihood vs. Financial Impact matrix. Tap any cell or item to inspect.
          </p>
        </div>

        {/* Legend */}
        <div className="grid grid-cols-2 sm:flex items-center gap-2 sm:gap-3 text-[10px] sm:text-[11px] font-medium font-mono">
          <span className="flex items-center gap-1.5 text-rose-800">
            <span className="w-2.5 h-2.5 rounded bg-rose-500 inline-block shrink-0" /> Critical (15-25)
          </span>
          <span className="flex items-center gap-1.5 text-amber-800">
            <span className="w-2.5 h-2.5 rounded bg-amber-500 inline-block shrink-0" /> High (10-14)
          </span>
          <span className="flex items-center gap-1.5 text-blue-800">
            <span className="w-2.5 h-2.5 rounded bg-blue-500 inline-block shrink-0" /> Moderate (5-9)
          </span>
          <span className="flex items-center gap-1.5 text-emerald-800">
            <span className="w-2.5 h-2.5 rounded bg-emerald-500 inline-block shrink-0" /> Low (1-4)
          </span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {/* Heatmap Grid / List Container */}
        <div className="lg:col-span-2">
          {/* Mobile List View alternative for super easy reading */}
          {mobileView === 'list' && (
              ...
              {risks.map((r) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-500">
                    ...
                    <span className="font-mono font-bold text-slate-800">{formatINR(r.expectedAnnualLoss)} EAL</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* 5x5 Matrix (Visible on desktop, or on mobile when grid is active with horizontal scroll) */}
          <div className={`${mobileView === 'list' ? 'hidden sm:block' : 'block'} overflow-x-auto pb-2`}>
            <div className="min-w-[480px] sm:min-w-0 relative pl-8 sm:pl-10 pb-6 sm:pb-8">
              {/* Y Axis Label */}
              <div className="absolute left-0 top-1/2 -translate-y-1/2 -rotate-90 text-[10px] sm:text-[11px] font-bold tracking-wider text-slate-500 uppercase">
                Likelihood (1 → 5)
              </div>

              {/* 5x5 Grid (Reversed L from 5 down to 1) */}
              <div className="grid grid-rows-5 gap-1.5 h-[320px] sm:h-[340px]">
              ...
              {/* X Axis Label */}
              <div className="text-center text-[10px] sm:text-[11px] font-bold tracking-wider text-slate-500 uppercase mt-2.5">
          ...
          {selectedRisk ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-5 text-slate-500 space-y-2">
              ...
              <p className="text-[11px] text-slate-500 leading-relaxed">
                Tap any cell in the 5×5 matrix or use the List toggle on mobile to inspect asset exposure and recommended controls.
              </p>
            </div>
          )}
        </div>
